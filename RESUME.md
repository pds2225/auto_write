...
> 최종 갱신: **2026-08-15** (머지 전 저장소 점검 완료, 머지 보류)
...
- 열린 PR은 #137, #136, #133, #132이며 모두 DRAFT다. 대상 지정 없이 머지하지 않았다.
- 다른 워크트리와 stash가 존재하므로 삭제·덮어쓰기 없이 보존한다.

## 다음 액션

1. 사용자가 지정한 PR의 변경·검증·머지 가능 상태를 확인한다.
2. 지정이 없으면 열린 DRAFT PR 목록과 보존해야 할 로컬 변경을 먼저 보고한다.
