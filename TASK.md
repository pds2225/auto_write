...
- 없는 숫자·실적을 채우지 않는다. `[확인필요]` 또는 공란 + 한 줄 요청.
- `cross_form_autofill` 전사는 사실 칸 KEEP. 서술 파트는 재작성(planner+writer). 전사 성공을 이 사용 케이스 DONE으로 치지 않는다.
...
- `cross_form_autofill` 안전철학 유지: 오매칭은 빈칸보다 나쁨, high confidence만 자동전사, 날조 0, 원본 미수정.
...
| AW-005 | 1순위 UX: 공고+빈 양식 먼저, 기존 계획서 선택, 가능 파트 즉시 미리보기, 부족한 정보만 요청 | 첫 화면에 자료 업로드칸 다 펼치기, 설문 먼저 |
