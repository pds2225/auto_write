...
[x] T-20260831-02 | 특정 지원사업은 원장에 저장하지 않는다. 원장 C가 제일 중요하다

에이전트 본문 금지: `# 0` LIST + 열린(`[ ]`/`[~]`) `8-1`만. T-20260814-02 8-1 전문은 생략. 제품 목표=`AW-005` 8-1 + `최우선 사용 케이스`. 웹앱 실행 정본=`웹앱 최종 요구사항_20260816`(승인 전 웹앱 코드 대기). 원장 **C가 최우선**(특정 지원사업 저장 금지). TASK 통째·CHANGELOG·닫힌 `[x]` DETAILS 금지.
...
- 원장 **C가 최우선** — 있는 기능을 실사용 가능하게. 특정 지원사업은 원장에 저장하지 않음
...
1. 이 파일 `# 0` LIST만 (20줄).
2. 열린(`[ ]`/`[~]`) 항목의 `8-1` 인용문만.
3. `docs/REQUEST_LEDGER.md` **C가 최우선**, 그다음 B. 특정 지원사업 이름 표(옛 A)는 보지 않음.
...
| T-20260831-02 | 특정지원사업은 저장하지마. 원장 씨가 제일 중요 |
...
개발 원장: `docs/REQUEST_LEDGER.md` **C → B**. 특정 지원사업 이름 표는 저장하지 않음.
...
- [ ] 신청 남은 손작업(사용자): 울산 상담위원 메일 발송, 수출바우처(비앤코) 사진·날인일·기존 HWP 글자 6곳. 한난 온랩은 2026-08-23 **접수 확인**.
...
- [V] **여성 1인 창조기업(마켓게이트)** — 사용자 2026-08-23 **안 함 / 취소**. 다시 쓰지 말 것.
...
- [V] KICXUP 1~3(추천·케이블텔레콤·신청서/동의서) **선정**(사용자 2026-08-23). 다시 작성하지 말 것.
...
STATUS_THIS_TURN: 원장 C를 최우선으로 고정. 특정 지원사업 이름 표 삭제. REQUEST_SOLVED=YES.
...
### 최종 결과
요청사항체크·개발 원장은 C(있는 기능 실사용)를 먼저 본다. 특정 공고·신청 건 이름을 `REQUEST_LEDGER`·RESUME 남은 일·요청체크 표에 저장하지 않는다.

### MUST
- `docs/REQUEST_LEDGER.md` 절 순서 C → B → A(저장 금지 한 줄)
- 특정 지원사업 이름 행을 원장에 다시 넣지 않음
- 스킬 훅에 요청 원문 그대로 (`AGENTS.md` §7)
- 물으면 채팅만. Google Docs·새 md 사업 표 금지
...
### FORBIDDEN
- 원장 A에 사업명·마감·선정 행을 다시 쌓기
- 요청사항체크에서 특정 지원사업 상태를 표로 저장·갱신
- `git add -A` · 원본 덮어쓰기
...
### DONE
- REQUEST_SOLVED=YES: 원장에서 특정 지원사업 이름 표가 없고 C가 맨 위다. 원문 두 줄이 TASK 8-1과 스킬 description에 있다.
