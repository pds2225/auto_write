...
### 8-15. CHECKPOINT — 2026-09-21 실사용 제품 흐름 정리

사용자 요청:

> 일단 오토라이트 브랜치에 제품흐름 실제사용가능하게 정리해줘

작업 브랜치:
- `feat/aw005-product-flow-actual-use-20260921`

정리 문서:
- `docs/PRODUCT_FLOW_ACTUAL_USE_20260921.md`

이번 체크포인트에서 제품 흐름을 다음처럼 한 줄로 고정:
- **공고 + 양식 → (선택) 기존 사업계획서 → 대목차/재료 분석 → 작성 가능한 파트 우선 → 재료 [1][2][3] 선택 → 자연어 지시 → Composition Plan 승인 → 해당 파트만 작성 → 부족자료 후요청 → 전체 대목차 완료 후 HWPX 조립 → LRule/Finalizer → 원본 폴더에 최종 파일**

현재 코드 기준 재사용 가능:
- `document_ingest`, `doc_text_extract`
- `announcement_analyzer`, `form_analyzer`
- `step2_output_contract.py`, `section_matcher.py`
- `cross_form_autofill`, `cross_form_fill`
- `bizplan_ai_writer`, `bizplan_autopilot`
- `hwpx_fill`, `hwp_fill_direct.py`, `hwpx_submit.py`
- `LRuleEnforcer`, `Finalizer`
- source-adjacent HWPX output (#184)

실사용 연결이 아직 필요한 부분:
- 첫 화면 공고/양식/기존계획서 3역할 intake
- 역할 사용자 확정
- 형식별 정확 locator + 페이지
- 모듈 카드/원문보기
- 선택재료 [1][2][3]
...
REQUEST_SOLVED=NO.
이번 브랜치는 제품 흐름 정리 체크포인트이며 AW-005 DONE이 아님.
