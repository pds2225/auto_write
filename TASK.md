...
에이전트 본문 금지: `# 0` LIST + 열린(`[ ]`/`[~]`) `8-1`만. T-20260814-02 8-1 전문은 생략. 제품 목표=`AW-005` 8-1 + `최우선 사용 케이스`. 원장 A/B 표만. TASK 통째·CHANGELOG·닫힌 `[x]` DETAILS 금지.
...
| AW-005 | 1순위 UX: 공고+빈 양식 먼저, 기존 계획서 선택, 가능 파트 즉시 미리보기, 부족한 정보만 요청. 서술 생성 허용+`생성` 인지. **파일 오기 전 초안 금지. P 개발 중에는 Problem만(요청 한 장). P 완료 후는 최우선이 이긴다. 사업자등록증은 첫 화면 필수 아님.** | 첫 화면에 자료 업로드칸 다 펼치기, 설문 먼저, 생성본을 출처 사실처럼 표시, 공고·양식 없이 초안 시작, 사업자등록증을 첫 화면 게이트로 쓰기, 신규 작성을 별도 제품으로 쪼개기 |
