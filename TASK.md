...
2. 아래 **기록된 사용자 요청 인덱스**만 읽는다. DETAILS `8-1` 전문은 그 TASK를 실행할 때만.
3. 실제출·옛 기능 요청은 `docs/REQUEST_LEDGER.md` A/B 표만.
4. T-20260814-02의 긴 `8-1`(명세 작성 지시) 전문은 기본 생략.

금지: TASK.md 통째 로드, CHANGELOG 통째, 닫힌 `[x]` DETAILS 재독. 이 표에 없는 옛 채팅은 실행 기준이 아니다.

### 기록된 사용자 요청 인덱스 (2026-08-16)

열린 LIST의 `8-1` 한 줄. 전문은 각 DETAILS.
...
| AW-009 | 내드라이브 요구사항 최종 사양으로 운영 웹앱 P0. 한·영 프롬프트 |
| T-20260814-02 | (8-1은 2026-08-14 **명세만 작성** 지시. 제품 목표는 아래 추가 요청 + AW-005) |
...
| `# 1` 이 절 | 과거 요청이 최우선. BPQ는 수단 |
| `조치 우선순위` | PARTIAL을 새 시스템으로 우회하지 말고 완성. REUSE → MERGE → EXTEND → CREATE IF GAP |

실제 제출물·옛 엔진 요청 (표만): `docs/REQUEST_LEDGER.md` A1~A6 / B1~B7 / C1.
...
| AW-005 | 1순위 UX: 공고+빈 양식 먼저, 기존 계획서 선택, 가능 파트 즉시 미리보기, 부족한 정보만 요청. 서술 생성 허용+`생성` 인지 | 첫 화면에 자료 업로드칸 다 펼치기, 설문 먼저, 생성본을 출처 사실처럼 표시 |
