...
| AW-005 | 1순위 UX: 공고+빈 양식 먼저, 기존 계획서 선택, 가능 파트 즉시 미리보기, 부족한 정보만 요청. 서술 생성 허용+`생성` 인지. **파일 오기 전 초안 금지(요청 한 장). 지금은 Problem만** | 첫 화면에 자료 업로드칸 다 펼치기, 설문 먼저, 생성본을 출처 사실처럼 표시, 공고·양식 없이 초안/BPQ-00 코드 시작 |
