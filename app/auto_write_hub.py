...
def cmd_fill(args: argparse.Namespace) -> int:
    ...
    if args.program_name:
        argv += ["--program-name", args.program_name]
    if args.document_type:
        argv += ["--document-type", args.document_type]
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_fill.add_argument("--name", help="레거시 호환 인자(새 기본 파일명에는 미사용)")
    p_fill.add_argument("--form-prefix", default="전문상담위원_참여신청서", help="레거시 호환 인자")
    p_fill.add_argument("--version", default=None, help="레거시 호환. 새 기본명은 vN 자동 증가")
    p_fill.add_argument("--program-name", default=None, help="최종 파일명 지원사업명")
    p_fill.add_argument("--document-type", default=None, help="최종 파일명 문서종류")
