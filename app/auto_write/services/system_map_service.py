...
class SystemMapService:
    ...
    NODE_SPECS = (
        ("web", "웹 운영 콘솔", "사용자가 문서와 규칙을 다루는 화면입니다.", "app/auto_write/operator_main.py", "FastAPI"),
        ("router", "DomainRouter", "요청이 어떤 문서 업무인지 분류합니다.", "app/auto_write/domains/domain_router.py", "class DomainRouter"),
        ("bizplan", "사업계획서 엔진", "사업계획서 작성 로직을 실행합니다.", "app/auto_write/services/bizplan_autopilot.py", "bizplan"),
        ("cross_form", "기존자료 활용", "기존 자료를 새 양식에 맞춰 활용합니다.", "app/auto_write/services/cross_form_autofill.py", "autofill"),
        ("lrule", "L 규칙 엔진", "canonical L 규칙 registry를 읽고 적용 상태를 관리합니다.", "app/auto_write/services/lrule_enforcer.py", "class LRuleEnforcer"),
        ("project", "문서 작성 서비스", "업로드 자료와 양식을 실제 문서 생성 과정에 연결합니다.", "app/auto_write/services/project_service.py", "class ProjectService"),
        ("render", "DOCX 렌더러", "작성 내용을 DOCX 결과물에 반영합니다.", "app/auto_write/services/render_service.py", "class RenderService"),
        ("converter", "HWP/DOCX 변환", "HWP·HWPX·DOCX 간 지원 가능한 변환을 수행합니다.", "app/auto_write/services/hwp_docx_convert.py", "def convert"),
        ("finalizer", "Finalizer", "최종 산출물 처리 단계입니다.", "app/auto_write/services/finalizer.py", "final"),
    )
    ...
    def _related_rules(self, file_path: str) -> list[str]:
    ...
    def nodes(self) -> list[dict]:
        rows = []
        for key, label, description, rel_path, symbol in self.NODE_SPECS:
            ...
            node = SystemNode(
                ...
                symbol=symbol,
                status=status,
                related_rules=self._related_rules(rel_path),
            )
        ...
        return [
            {
                "key": "write",
                "label": "문서 작성",
                "description": "공고·양식·기존자료를 한 화면에서 받아 실제 기존 작성 엔진으로 연결합니다.",
                "steps": [
                    ...
                    {"key": "render", "label": "DOCX 반영", "service": "RenderService", "node": "render"},
                ],
            },
            {
                "key": "revise",
                ...
                "steps": [
                    ...
                    {"key": "render", "label": "새 결과 저장", "service": "RenderService", "node": "render"},
                ],
            },
            {
                "key": "convert",
                "label": "문서 변환",
                "description": "입력 확장자와 목표 형식에 따라 기존 변환 서비스를 호출합니다.",
                "steps": [
                    {"key": "upload", "label": "문서 입력", "service": "Web", "node": "web"},
                    {"key": "detect", "label": "형식 확인", "service": "Web", "node": "web"},
                    {"key": "convert", "label": "변환", "service": "hwp_docx_convert", "node": "converter"},
