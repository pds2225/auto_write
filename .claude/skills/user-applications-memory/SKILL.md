...
이 사용자는 **문서를 안 읽는다.** 신청 이력은 에이전트 메모리에 두고, 나중에 물으면 **채팅 한 번에** 답한다. Drive에 정리본을 만들어 주면 실패다.
...
4. 폴더명 `제출완료` ≠ 사이트 접수 확정. **선정**만 확정으로 적는다. STAR=내비 선정. 대구 스타전에 STAR를 수출로 적은 칸은 오류.
...
원장: `docs/clients/user_applications.md`. 내비 카드: `docs/clients/dobonevi_card.md`. 위키: [[User applications ledger]] · [[Chat not Google Docs]].
