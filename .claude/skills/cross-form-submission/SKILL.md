...
- **최종 사용자 HWPX 위치** — 기본은 빈 양식 B(원본)의 폴더에 `{지원사업명}_{문서종류}_{MMDDHH} vN.hwpx`. 같은 이름이면 vN 자동 증가. 중간본·리포트는 `_workspace/` 유지.
...
- 최종 HWPX는 별도 출력 미지정 시 원본 B와 같은 폴더에 통일 파일명으로 저장. 원본 A·B는 절대 덮어쓰지 않는다.
