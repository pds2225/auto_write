...
_HWP_EXTS = {".hwp", ".hwpx"}
_SUPPORTED_EXTS = {".docx", ".hwp", ".hwpx"}  # H7: 타깃 지원 확장자(값을 써넣어야 함)
# 소스는 PDF 도 허용(읽기 전용 텍스트 추출). 타깃은 _SUPPORTED_EXTS 로 따로 검사.
_SOURCE_EXTS = {".docx", ".hwp", ".hwpx", ".pdf"}
...
def _cluster_members(norm_key: str) -> list[tuple[str, str]]:
    """norm_key 가 속한 동의어 클러스터의 멤버를 ``[(정규화멤버, 표시멤버), …]`` 로 반환.

    소스(PDF)가 타깃과 다른 동의어 라벨로 쓰여 있어도(예: 타깃 '대표자' ↔ 소스 '성명')
    앵커에 클러스터 멤버를 함께 넣어 잡기 위함이다. 클러스터 미등록이면 빈 리스트.
    """
    rep = _CLUSTER_OF.get(norm_key)
    if rep is None:
        return []
    members: list[tuple[str, str]] = []
    seen: set[str] = set()
    for cluster in SYNONYMS:
        if _key(cluster[0]) != rep:
            continue
        for alias in cluster:
            nk = _key(alias)
            if nk and nk not in seen:
                seen.add(nk)
                members.append((nk, alias))
    return members
...
# --- 추출: PDF(플랫 텍스트) 소스에서 식별항목 라벨→값 ----------------------------
# 완성본이 PDF 인 경우, 표 구조를 잃은 플랫 텍스트에서 '짧은 식별항목'(기업명·대표자·
# 연락처 등)만 타깃-라벨 앵커 방식으로 끄집어낸다. 에세이 본문(긴 서술)은 값 길이
# 폐기 가드(_MAX_VALUE_LEN)로 차단한다 — 식별항목만 안전하게 전사(과대 값 폐기).

_MAX_LABEL_LEN = 20   # 라벨 후보 최대 길이(문자)
_MAX_VALUE_LEN = 60   # 식별항목 값 최대 길이 — 초과 시 폐기(에세이 본문 차단)


def _find_label_spans(line: str, anchor_keys: set[str]) -> list[tuple[int, int, str, str]]:
    """한 줄에서 앵커 라벨 스팬을 왼쪽부터 '최장일치'로 찾는다.

    반환: ``[(start, end, key, raw_label_text), …]`` (start 오름차순).
    알고리즘: i 를 진행하며 공백을 skip. 각 i 에서 j 를 1씩 늘려 line[i:j] 를 _key 로
    정규화한 값이 anchor_keys 에 있으면 best_end/best_key 갱신(가장 긴 매칭 채택,
    j-i <= _MAX_LABEL_LEN 까지). best 가 있으면 스팬을 추가하고 i=best_end 로 점프,
    없으면 i+=1.

    _key 는 괄호·공백·장식을 제거하므로 "기업(예정)명"→"기업명", "명     칭"→"명칭"
    이 자연히 매칭된다(실측 확인).
    """
    ...
    while i < n:
        ...
        best_key = ""
        j = i + 1
        while j <= n and (j - i) <= _MAX_LABEL_LEN:
            k = _key(s[i:j])
            if k in anchor_keys:
                best_end = j
                best_key = k
            j += 1
        if best_end > i:
            spans.append((i, best_end, best_key, s[i:best_end]))
            i = best_end
        else:
            i += 1
    return spans
...
def _extract_source_from_text(
    text: str, anchor: dict[str, str]
) -> tuple[dict[str, str], dict[str, str]]:
    """PDF 플랫 텍스트에서 식별항목 (라벨,값) 쌍을 앵커 방식으로 추출한다.

    anchor: ``{정규화키: 표시라벨}``. 반환은 ``(정규화필드, 원본라벨맵)`` 으로
    _extract_source 와 동일 형태라 match_fields 에 그대로 넣을 수 있다.

    각 줄에서 _find_label_spans 로 라벨 스팬들을 찾고, 각 라벨의 값 =
    line[그 라벨 end : 다음 라벨 start](마지막은 줄끝)를 strip.

    폐기 가드(전부 적용 — 오매칭<빈칸):
      - 값이 비었거나 _is_fill_blank(값)이면 skip(빈칸/밑줄/점).
      - _key(값)가 anchor_keys 에 있으면 skip(값이 또 다른 라벨 → 오염 차단).
      - len(값) > _MAX_VALUE_LEN 이면 skip(에세이 본문 등 과대값).
      - 같은 정규화키는 처음 값 유지(first-wins, _extract_source 와 동일).
    """
    ...
    for line in (text or "").splitlines():
        spans = _find_label_spans(line, anchor_keys)
        if not spans:
            continue
        for idx, (start, end, key, raw_label) in enumerate(spans):
            value_end = spans[idx + 1][0] if idx + 1 < len(spans) else len(line)
            value = line[end:value_end].strip()
            if not value or _is_fill_blank(value):
                continue                       # 빈칸/밑줄/점 → 값 아님(날조 0)
            if _key(value) in anchor_keys:
                continue                       # 값이 또 다른 라벨 → 오염 폐기
            if len(value) > _MAX_VALUE_LEN:
                continue                       # 과대값(에세이 본문) 폐기
            if key not in fields:              # first-wins
                fields[key] = value
                originals[key] = SubmittableFiller._norm(raw_label)
...
def autofill_from_source(
    ...
    *,
    ...
    # H7: 지원 확장자 화이트리스트. 소스는 PDF 도 허용(읽기 전용 텍스트 추출),
    # 타깃은 값을 써넣어야 하므로 .docx/.hwp/.hwpx 만(PDF 금지 유지).
    for role, p, allowed, hint in (
        ("소스", source, _SOURCE_EXTS, ".docx/.hwp/.hwpx/.pdf"),
        ("타깃", target, _SUPPORTED_EXTS, ".docx/.hwp/.hwpx"),
    ):
        if p.suffix.lower() not in allowed:
            report.ok = False
            report.notes.append(
                f"{role} 비지원 확장자({p.suffix or '없음'}) — {hint} 만 지원")
            return report
        ...
        try:
            ...
            if source_is_pdf:
                ...
                if not (text or "").strip():
                    report.notes.append(
                        "PDF 텍스트 추출 결과 없음(스캔 PDF 가능)")
                    src_fields, src_originals = {}, {}
                else:
                    src_fields, src_originals = _extract_source_from_text(text, anchor)
                    report.notes.append(
                        f"PDF 소스 텍스트 추출(식별항목 앵커 방식, {len(text)}자)")
            else:
