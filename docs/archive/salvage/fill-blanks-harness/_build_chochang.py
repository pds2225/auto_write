...
def cmd_fill(argv: list[str]) -> None:
    """빈칸 채우기 공통 골격 CLI.

    사용법: python _build_chochang.py fill <input.docx> <output.docx> [--plan plan.json]
    --plan 은 dict(JSON) 파일 경로. 주어지면 로드해서 plan 으로 채운다. 없으면 안전 복사.
    원본은 절대 덮어쓰지 않으며(출력==입력이면 에러), 후처리 전 results/backup 에 백업한다.
    """
    import json as _json
    from auto_write.services.blank_fill_service import fill_blanks

    if len(argv) < 2:
        print("사용법: python _build_chochang.py fill <input.docx> <output.docx> [--plan plan.json]")
        return
    in_docx = argv[0]
    out_docx = argv[1]
    plan = None
    if "--plan" in argv:
        ...
        print("plan      :", plan_path, "| 키:", list(plan.keys()))

    report = fill_blanks(in_docx, out_docx, plan=plan)

    print("=" * 60)
    print("FILL BLANKS REPORT")
    print("input     :", report["input_docx"])
    print("output    :", report["output_docx"])
    print("backup    :", report["backup_dir"])
    print("plan 적용 :", report["plan_applied"], "| 채움:", report["filled"])
    if not report["filled"]:
        print("사유      :", report.get("reason", "-"))
    else:
        # 채운 항목 수(SubmittableFiller report 키)
        for k in ("identity_filled", "overview_filled", "rows_rewritten",
                  "replacements", "paragraphs_filled", "guides_blanked",
                  "residual_cleaned"):
            if k in report:
                print(f"  {k:18}: {report[k]}")
        for note in report.get("notes", []):
            print("  NOTE:", note)
    residual = report.get("residual_remaining", [])
    print("잔존 더미 :", len(residual))
    for r in residual[:25]:
        print("   !", r)
    print("output exists:", Path(report["output_docx"]).exists())
