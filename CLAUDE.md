...
| 2026-07-13 | 채움 스크립트 근본버그 수정 — 행 삽입 시 rowAddr/rowCnt 미정합 | 수정 app/scripts/patch_profile_v5_visual.py(_row_addr 헬퍼 + 삽입 정합) | 위 hwpx 표 격자 결함의 **근본 원인 차단**: `patch_profile_v5_visual` 경영분야 행 삽입이 `copy.deepcopy(rows[2])` + `addnext` 만 하고 **삽입 위치 이후 논리행 rowAddr +1·rowCnt +1 을 안 함** → 새 행이 아래 행과 rowAddr 중복 → 격자 겹침 → 한글 열기 거부. 수정: 삽입 전 이후 행 rowAddr +1 밀고 새 행 rowAddr 고정 + rowCnt +1(병합 rowSpan 보존). 합성 검증: 병합 포함 표에서 옛 로직=격자 깨짐 / 새 로직=정상·rowCnt 4→5. (엔진 finalize repair_grid 안전망과 이중 방어) |