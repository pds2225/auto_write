...
## 8. 에이전트가 띄운 임시 파일·창·프로세스 정리

에이전트가 **자기 작업을 위해 새로 띄운 것만** 확인이 끝나면 자동으로 정리한다.

- 조사·검증용으로 에이전트가 직접 연 임시 파일, 미리보기, diff, 로그, 브라우저/에디터 창은 확인 완료 후 닫는다.
- 테스트·렌더·검증을 위해 에이전트가 직접 시작한 임시 프로세스와 뷰어는 결과 확인 후 종료한다.
- background process는 작업 완료 시 필요 여부를 확인하고, 불필요하면 종료한다.
- 사용자가 원래 열어 둔 창·프로세스·파일은 닫거나 종료하지 않는다.
- 저장되지 않은 변경이 있는 파일·창은 자동으로 닫지 않는다.
- 사용자 소유인지 에이전트가 띄운 것인지 불명확하면 건드리지 않는다.
- 필요한 로그·테스트 증거·산출물은 먼저 보존한 뒤 정리한다.
- 파일 자체는 삭제하지 않는다. 임시 산출물 삭제는 기존 cleanup 규칙이 명확할 때만 수행한다.
