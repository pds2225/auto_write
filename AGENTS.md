...
## 8. 검사용 파일·창·프로세스 자동 정리

에이전트가 **자기 작업을 위해 새로 띄운 것만** 확인이 끝나면 자동으로 정리한다.

- 조사·검증을 위해 에이전트가 직접 연 임시 파일, 미리보기, diff, 로그, 브라우저/에디터 창은 확인 완료 후 닫는다.
- 테스트·렌더·검증을 위해 에이전트가 직접 시작한 임시 프로세스와 뷰어는 결과 확인 후 종료한다.
- background process는 작업 완료 시 실제 필요 여부를 확인하고, 불필요하면 종료한다.
- 파일 자체는 삭제하지 않는다. 임시 산출물 삭제가 필요하면 해당 작업의 기존 cleanup 규칙이 명확한 경우에만 수행한다.
- 저장되지 않은 변경이 있는 파일·창은 자동으로 닫지 않는다.
- 사용자가 원래 열어 둔 창, 프로세스, 파일은 닫거나 종료하지 않는다.
- 어떤 창/프로세스가 사용자 소유인지 에이전트가 띄운 것인지 확실하지 않으면 건드리지 않는다.
- 장시간 자동개발에서는 각 checkpoint와 세션 종료 직전에 에이전트가 띄운 임시 창·프로세스를 점검해 정리한다.
- 자동 정리 때문에 테스트 증거·로그·산출물이 사라지면 안 된다. 필요한 증거는 먼저 파일/리포트/RESUME에 보존한 뒤 정리한다.


## 9. 긴 GPT/개발 프롬프트는 .md 파일로 제공

사용자에게 전달할 GPT/Codex/Claude/Astra 실행 프롬프트가 웹페이지 한 화면을 넘어가는 길이면 채팅 본문에 길게 붙이지 않는다.

- 한 화면 이내의 짧은 프롬프트는 채팅 본문에 직접 제공한다.
- 한 화면을 넘는 긴 개발·Codex·Claude·Astra 실행 프롬프트는 기본적으로 **1개 `.md` 파일**로 제공한다.
- `.md` 파일에는 사용자가 그대로 복사·실행할 수 있는 **전체 프롬프트**를 넣는다.
- 채팅 본문에는 목적, 입력 위치/대상, 파일 링크만 간단히 적는다.
- 특별한 이유가 없으면 긴 프롬프트를 여러 조각으로 나누지 않는다.
- 단순 설명·요약·짧은 명령까지 억지로 파일화하지 않는다.
- 저장소 작업 중 생성한 프롬프트 파일을 repo 정본으로 취급하지 않는다. 사용자가 repo 저장을 명시하지 않은 한 TASK.md/제품 코드와 분리한다.
...
| 날짜 | 변경 내용 | 사유 |
|------|----------|------|
| 2026-09-23 | §9 긴 실행 프롬프트는 한 화면 초과 시 1개 `.md` 파일로 제공 | 사용자가 긴 GPT/Codex/Claude/Astra 프롬프트의 기본 전달형식을 파일로 고정 요청 |
| 2026-09-18 | §8 검사용 파일·창·프로세스 자동 정리 규칙 추가 | 사용자가 에이전트가 띄운 파일/창을 확인 후 자동으로 닫도록 요청 |
